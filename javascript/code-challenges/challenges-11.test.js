...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(newArr => newArr.filter(value => (typeof value === 'number' && value % 5 === 0)).map(value1 => Math.pow(2, value1)));
};
...
let findMaleAndFemale = (data) => {
  ...
  let resArr=[];
  data.map(word =>{
   if(word.gender==='female' || word.gender==='male'){
     resArr.push(word.name);
   }
  })
  
  result = resArr.join(' and ');
...
let findShortest = (data) => {
  let height = data[0];
  data.map(item => {
    if(parseInt(item.height) <= parseInt(height.height)){
      height = item;
    }
  });
  return height.name;
