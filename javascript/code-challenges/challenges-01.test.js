...
Write aaa function named `addOne` that takes an array of numbers, and returns a new array of the numbers, incremented by 1.
...
const greeting = (word) => {
    const newArr = [];
    word.forEach(element => newArr.push(element.toUpperCase() + "!"));
    return newArr;
};

const speaker = (words, callback) => {
    const arr = callback(words)
    return arr;
};
